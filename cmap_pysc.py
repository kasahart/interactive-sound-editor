...
def set_create_root_element(
    root_element, fig: matplotlib.figure
) -> matplotlib.figure:

    div = js.document.createElement("div")
    root_element.appendChild(div)

    def cre(self): return div
...
class AudioViewPs(c_map.AudioView):
    ...
    def setup_ui(self):
        ...
        self.audio_player = pn.pane.Audio(np.zeros(0), sample_rate=self.viewModel.model.sr)
        self.inv_audio_player = pn.pane.Audio(np.zeros(0), sample_rate=self.viewModel.model.sr)
        self.audio_player_pane = pn.Row(self.audio_player, self.inv_audio_player)
        
        self.target_db_slider = pn.widgets.FloatSlider(name='Target dB', start=0, end=60, step=1, value=20)
        self.target_db_slider.param.watch(self.on_target_db_slider_value_changed, 'value_throttled') 
        self.reduction_db_slider = pn.widgets.FloatSlider(name='Reduction dB', start=0, end=60, step=1, value=20)
        self.reduction_db_slider.param.watch(self.on_reduction_db_slider_value_changed, 'value_throttled')    
        ...
        self.save_button = pn.widgets.FileDownload(
            callback= pn.bind(self.save_audio),
            filename='output.wav'
        )
        ...
        self.vb_bf = pn.Row(self.go_back_button, self.go_forward_button, self.save_button)
        
        self.output_area = pn.Column(self.audio_player_pane, self.vb_bf, self.vb_select, self.vb_reduction, self.fig_pane)
