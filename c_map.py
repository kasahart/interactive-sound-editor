...
cmap_target = mcolors.LinearSegmentedColormap.from_list(
    "", [(0, 0, 0, 0), (1, 0, 0, 0.5)]
)


def softmask(X: np.ndarray, Y: np.ndarray, *, power: float = 2.0) -> np.ndarray:
    mask = X / (X + Y)
...
def compute_harmonic(S: np.ndarray, win_harm: int) -> np.ndarray:
    harm_shape = [1] * S.ndim
...
def compute_percussive(S: np.ndarray, win_perc: int) -> np.ndarray:
    perc_shape = [1] * S.ndim
...
def compute_harmonic_mask(
    S: np.ndarray, kernel_size: int = 31, power: float = 2.0, margin: float = 1.0
) -> np.ndarray:
    S = np.abs(S)
    harm = compute_harmonic(S, kernel_size)
    perc = compute_percussive(S, kernel_size)
    mask_harm = softmask(harm, perc * margin, power=power)
    return mask_harm


def compute_percussive_mask(
    S: np.ndarray, kernel_size: int = 31, power: float = 2.0, margin: float = 1.0
) -> np.ndarray:
    S = np.abs(S)
    harm = compute_harmonic(S, kernel_size)
    perc = compute_percussive(S, kernel_size)
    mask_perc = softmask(perc, harm * margin, power=power)
    return mask_perc
...
class AudioModel:
    def __init__(
        ...
        self.f, self.t, self.Sxx = stft(
            self.audio_normalized,
            self.sr,
            nperseg=self.n_fft,
            noverlap=self.hop_length,
            nfft=self.win_length,
            window=self.window,
        )
...
class AudioViewModel:
    ...
    def apply_harmonic_enhancement(self):
        start_time_index, end_time_index, start_freq_index, end_freq_index = (
            self._limited_area()
        )
    ...
    def apply_zscore_mask(self, threshold=2):
        start_time_index, end_time_index, start_freq_index, end_freq_index = (
            self._limited_area()
        )
        ...
        mean = np.mean(Sxx, axis=1, keepdims=True)
        std = np.std(Sxx, axis=1, keepdims=True)
    ...
    def get_audio_from_spectrogram(
        self, apply_mask=True, start_time=None, end_time=None
    ):
        if apply_mask:
    ...
    def update_history(self, audio, sr):
        ...
        model = AudioModel(
            data=self.model.inverse_scaling(audio), sr=sr, norm=self.model.norm
        )
        ...
        logger.debug(
            f"update_history: Sxx_history:{len(self.model_history)}, Sxx_index:{self.model_index}"
        )
    ...
    def is_selected_area(self):
        is_selected = self.selected_area != [0, 0, 0, 0]
        logger.debug("Selected area: " + str(is_selected))
        return is_selected
    ...
    def create_target_mask(self, invert=False):
        ...
        if invert:
            temp = self.get_mask_inv_Sxx_log()[
                start_freq_index:end_freq_index, start_time_index:end_time_index
            ]
        else:
            temp = self.get_mask_Sxx_log()[
                start_freq_index:end_freq_index, start_time_index:end_time_index
            ]
        ...
        target_mask[start_freq_index:end_freq_index, start_time_index:end_time_index][
            temp_mask
        ] = True
...
class AudioView:
    ...
    def setup_fig(self):
        ...
        self.ax.append(
            self.fig.add_subplot(gs[3], sharex=self.ax[0], sharey=self.ax[0])
        )
        # 3番目のサブプロットを作成し、y軸をax1と連動
        self.ax.append(self.fig.add_subplot(gs[1], sharey=self.ax[0]))
        # 4番目のサブプロットを作成し、x軸とy軸をそれぞれax2とax3と連動
        self.ax.append(
            self.fig.add_subplot(gs[4], sharex=self.ax[2], sharey=self.ax[0])
        )
        ...
        cmap_settings = {
            ...
            "rasterized": True,
            "extent": [
                self.viewModel.model.t.min(),
                self.viewModel.model.t.max(),
                self.viewModel.model.f.min() / 1e3,
                self.viewModel.model.f.max() / 1e3,
            ],
        }
        ...
        self.inv_selector = RectangleSelector(
            self.ax[1],
            self.onselect,
            useblit=True,
            button=[1],  # 左クリックのみ有効
    ...
    def setup_ui(self):
        ...
        self.target_db_slider = widgets.IntSlider(
            min=0,
            max=60,
            step=1,
            value=20,
        ...
        self.target_db_slider.observe(
            self.on_target_db_slider_value_changed, names="value"
        )
        self.reduction_db_slider = widgets.IntSlider(
            min=0,
            max=60,
            step=1,
            value=20,
            continuous_update=False,
            description="Reduction (dB):",
        )
        self.reduction_db_slider.observe(
            self.on_reduction_db_slider_value_changed, names="value"
        )
        ...
        self.enhance_percussive_button = widgets.Button(
            description="Enhance Percussive"
        )
        self.enhance_percussive_button.on_click(self.enhance_percussive)
        self.enhance_harmonic_button = widgets.Button(description="Enhance Harmonic")
        self.enhance_harmonic_button.on_click(self.enhance_harmonic)
        self.enhance_anomaly_button = widgets.Button(description="Enhance Anomaly")
    ...
    def onselect(self, eclick, erelease):
        ...
        if self.ax[0] == eclick.inaxes or self.ax[1] == eclick.inaxes:
            # 選択された領域の始点と終点を記録
            self.viewModel.select_area(
                eclick.xdata, eclick.ydata * 1000, erelease.xdata, erelease.ydata * 1000
            )
            self.state_changed = True
    ...
    def enhance_percussive(self, b=None):
        self.viewModel.apply_percussive_enhancement()
        self.state_and_audio_change()
        logger.debug("Percussive enhancement applied")

    def enhance_harmonic(self, b=None):
        self.viewModel.apply_harmonic_enhancement()
        self.state_and_audio_change()
        logger.debug("Harmonic enhancement applied")

    def enhance_anomaly(self, b=None):
    ...
    def update_audio_player(self):
        ...
        with self.audio_output:
            ...
            display(org_audio_player, audio_player, inv_audio_player)
